...
2. **이벤트 약 3,000건** 생성 (`--rate 50 --duration 60`, 60초 — 루프 오차로 ±50건 변동)
...
**이유**: 평가자가 `docker compose up`을 여러 번 실행해도 매번 **약 3,000건**(±50)으로 초기화된 상태에서 시작하기 위함. 볼륨이 있으면 이전 데이터가 누적되어 차트 수치가 매번 달라집니다.
